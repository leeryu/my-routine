...
const context = {
  ...
  caches: { async open(){return cache;}, async keys(){return ['routine-v7','routine-v8','routine-v9'];}, async delete(k){deleted.push(k);return true;}, async match(req){const key=typeof req==='string'?req:req.url; return store.get(key);}},
...
(async()=>{
  ...
  await dispatch('activate'); assert.deepEqual(deleted.sort(),['routine-v7','routine-v8']); assert.ok(context.claimed); console.log('ok - update removes old caches and claims clients');
