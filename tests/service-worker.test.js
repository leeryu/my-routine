...
const context = {
  ...
  caches: { async open(){return cache;}, async keys(){return ['routine-v6','routine-v7','routine-v8'];}, async delete(k){deleted.push(k);return true;}, async match(req){const key=typeof req==='string'?req:req.url; return store.get(key);}},
...
(async()=>{
  await dispatch('install'); assert.ok(!added.includes('./js/rehab.js')); assert.ok(added.includes('./js/completion-sync.js')); assert.ok(context.skipped); console.log('ok - fresh install precaches the simplified app modules');
  await dispatch('activate'); assert.deepEqual(deleted.sort(),['routine-v6','routine-v7']); assert.ok(context.claimed); console.log('ok - update removes old caches and claims clients');
