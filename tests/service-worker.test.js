...
const context = {
  ...
  caches: { async open(){return cache;}, async keys(){return ['routine-v5','routine-v6','routine-v7'];}, async delete(k){deleted.push(k);return true;}, async match(req){const key=typeof req==='string'?req:req.url; return store.get(key);}},
...
(async()=>{
  ...
  await dispatch('activate'); assert.deepEqual(deleted.sort(),['routine-v5','routine-v6']); assert.ok(context.claimed); console.log('ok - update removes old caches and claims clients');
